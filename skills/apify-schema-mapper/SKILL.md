...
- 若抓取失败（404/超时），明确报告该链接不可用并跳过，不臆造字段。
...
- **交付物只有 `input_schema.json` + `output_schema.json` 两个文件**（`.md`、映射说明等均不落盘）。
  映射说明在回复里给出：字段数、select/checkbox 来源、required 推断声明、输出来源（含"拟定"标注）、
  concurrency 说明、已知限制。
...
- `north-carolina-sos-business-search`：11 输入（stringList/switch/number/textarea），
  25 输出列（取 README `Example output`）。
