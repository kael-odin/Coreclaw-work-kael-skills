...
- **问与答要一致**。❌ 问"能拿到某个创作者的完整视频历史吗？"答"设为 `oldest` 并调大 `max_results`……大约能返回 400–500 条"——答案其实在说"拿不全"，和问的"完整历史"矛盾。✅ 先正面回答"无法保证完整，TikTok 对单个主页有上限"，再给"尽量多拿"的建议。通则：如果答案是"不能/有限制"，问题又假设了"能"，必须先把诚实的否定结论摆出来，再给折中方案。
