...
**emoji 双列表**列出每个输出字段，严格映射 `output_schema.json`。表只有两列、每格一个 emoji + 人类可读的语义化描述（把若干相关字段归并成一句卖点），**不列字段名、不列类型、不列 enum**。范式（来自已发布 Google Maps Scraper 与 TikTok Profile Scraper）：

```
|                                                                              |                                                              |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 👤 创作者主页（用户名、昵称、简介、头像、地区、粉丝与点赞数）              | 🎬 视频元数据（时长、尺寸、宽高比）                          |
| ▶️ 播放数                                                                    | ❤️ 点赞数                                                    |
```

- **不写类型列**。普通用户不关心 `object`/`array`/`integer`，类型信息留在「字段详解」按需展开。
- **不写裸字段名列**。把字段语义翻译成用户能懂的一句话（`diggCount` → "❤️ 点赞数"），相关字段可归并（`createTime`+`createTimeISO` → "📅 发布时间（Unix 时间戳与 ISO 8601 两种格式）"）。
- **emoji 前置**，每格一个，做视觉锚点。两列宽度大致均衡，奇数条目时最后一行左列留空右列补内容（或反之），不要出现半空行。
- **正文不堆字段名**。散文讲"做什么、怎么用"，用一句语义描述代替字段名罗列；字段名只出现在「输入」「输出」章节与 JSON 示例里。
- 严格的逐字段名清单放在「输出」章节的「完整 JSON 记录」与「字段详解」里——那里才是字段名/类型该出现的地方。

随后一段"为最大化采集效果，X 提供以下能力"的 bullet：地点与联系数据、可直接导出的格式（8 种）、集成能力（webhook、MCP）、可选增强项。

这张表是数据卖点——表中提到的每一项都必须能追到 `output_schema.json` 的某个字段，只是不把字段名直接写出来。
...
### 7. `## ❓ 常见问题` — 基于脚本真实能力的 Q&A

6-10 条，每条答案基于脚本真实能力，正面呈现脚本能做到什么。**不设弱项、计费、并发类问答**：不写"拿不全""哪些字段为空""每地上限"等脚本不足，不写运行成本/计费方式，不写并发配置/如何提高采集速度。标准集（按脚本裁剪）：
...
- 编造图片 URL / demo 链接。
- 中文标题与英文结构不对应（破坏锚点对称）。
- 把脚本弱项写进 README（"拿不全""每地上限 N 条""某字段恒为空"）——只呈现能力，不主动暴露不足。
- 写计费/成本说明（"增强开关有运行时成本""每次运行消耗余额"）。
- 写并发说明（"并发越高采集越快""提高 max_concurrency"）。
